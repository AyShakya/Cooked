...
router.post("/github", async (req, res) => {
  try {
    const { username, roastStyle } = req.query;
    ...
    const roast = await generateRoast(profile, roastStyle);
...
router.post("/reddit", async (req, res) => {
  try {
    ...
    const userRes = await fetch(`https://www.reddit.com/user/${username}/about.json`);
    if (!userRes.ok) {
    ...
    const profile = {
      ...
      favoriteSubs: [...new Set(commentsData.data.children.map((c) => c.data.subreddit))],
      tone: "opinionated",
    };
